# based on https://github.com/josegonzalez/josediazgonzalez.com/blob/master/_plugins/post_type.rb
  ...
  class PostType < Page
    ...
    def initialize(site, dir, folder, post_path, post_type)
      ...
      if !self.data.has_key?('layout')
        self.data['layout'] = site.config[post_type + '_index_layout'] || post_type
      end
      ...
      unless ['.textile', '.markdown', '.html', '.md'].include?(ext)
        ext = '.textile'
      end
  ...
  class PostTypeGenerator < Generator
    ...
    def generate(site)
      ...
      site.config['post_types'].each do |post_type|
        ...
        default_folder   = '_' + post_type
        dir_key          = post_type + '_dir'

        layout_index = site.config[post_type + '_index_layout'] || post_type
    ...
    def get_files(folder)
      ...
      Find.find(folder) do |file|
        if file=~/.markdown$/ or file=~/.textile$/ or file=~/.md$/
          files << file
        end
      end
